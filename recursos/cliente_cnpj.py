...
from modelos.cliente_cnpj import ClienteModel
import psycopg2
...
class ClientesCnpj(Resource):
    def get(self):
        ...
        if not parametro.get('cliente_nome'):
            consulta = "SELECT * FROM pessoajuridica ORDER BY cliente_id ASC LIMIT %s OFFSET %s"
            tupla = tuple([parametro[chave] for chave in parametro])
            cursor.execute(consulta, tupla)
            cursor.execute(consulta, tupla)
            resultado = cursor.fetchall()
        else:
            consulta = "SELECT * FROM pessoajuridica WHERE (cliente_nome = %s) ORDER BY cliente_id ASC LIMIT %s OFFSET %s"
            tupla = tuple([parametro[chave] for chave in parametro])
            cursor.execute(consulta, tupla)
            resultado = cursor.fetchall()
        pessoajuridica = []
...
class ClienteCnpj(Resource):
    def get(self, cliente_id):
        cliente = ClienteModel.achar_cliente(cliente_id)
        if cliente:
            return cliente.json()
        return {'message': 'Cliente não encontrado.'}, 404  # return make_response(render_template("cliente,html", message="Cliente não encontrado"), 404)

    def post(self):
        data = argumentos.parse_args()
        nome_encontrado = ClienteModel(**data)
        if nome_encontrado.achar_cliente_cnpj(data['cliente_cnpj']):
            return {'message': 'Cliente já cadastrado'}, 409  # return make_response(render_template("client.html", message="Cliente já cadastrado"), 409)
...
class ClienteCnpjs(Resource):
    ...
    def delete(self, cliente_id):
        ...
        if cliente:
            ...
            return {'message': 'Cliente deletado com sucesso.'}  # return make_response(render_template("client.html", message="Cliente deletado com sucesso."), 200)
